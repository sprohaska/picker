...
PickerImp.prototype._dispatch = function(req, res, bypass) {
  ...
  function processNextMiddleware (err) {
    // Hack to pass errors such as body size limit violations to the route
    // handler without implementing full express-style error handling (route
    // handlers with an additional err argument).
    if(err) {
      req.error = err
    }
